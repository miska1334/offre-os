...
const Results = {
  ...
  renderTitres(titres) {
    ...
    if (!Array.isArray(titres) || titres.length === 0) {
      container.innerHTML = '<p style="color:var(--text-secondary)">Les titres n\'ont pas pu être générés. Réessaie.</p>';
      return;
    }
  ...
  renderArchitecture(text) {
    ...
    container.innerHTML = `
      <div style="white-space:pre-wrap;word-break:break-word;font-size:.92rem;color:var(--text-secondary);line-height:1.8">${this.escape(text)}</div>
  ...
  renderEmails(emails) {
    ...
    if (!Array.isArray(emails) || emails.length === 0) {
      container.innerHTML = '<p style="color:var(--text-secondary)">Les emails n\'ont pas pu être générés.</p>';
      return;
    }
